			...
			<div class="am-cf admin-main">
				...
				<div class="admin-content">
					...
					<div class="am-cf am-padding">
						<div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">岗位调动报表</strong> <small></small></div>
					</div>
							...
							<form class="am-form">
								<table class="am-table am-table-striped am-table-hover table-main">
									...
									<tr>
										...
										<th class="table-title">调转前岗位</th>
										<th class="table-type">姓名</th>
										<th class="table-date">性别</th>
										<th class="table-set">部门</th>
										<th class="table-author">调转后岗位</th>
										<th class="table-author">调转日期</th>
										<th class="table-author">调转原因</th>
									</tr>

									<c:forEach items="${allPost_trans}" var="post_trans" varStatus="i">
										<tr>
											<td>${i.index+1 }</td>
											<td>${post_trans.employee.post.post_id } </td>
											<td>${post_trans.employee.emp_name }</td>
											<td>
												<c:if test="${post_trans.employee.emp_sex=='1' }">男</c:if>
												<c:if test="${post_trans.employee.emp_sex=='2' }">女</c:if>
											</td>
											<td> ${post_trans.employee.department.dep_name } </td>
											<td>${post_trans.post.post_name } </td>
											<td>
												<fmt:formatDate value="${post_trans.trans_date }" pattern="yyyy-MM-dd" />
											</td>
											<td>
												${post_trans.trans_reason}
