	...
	-->
		...
		<script type="text/javascript">
			function delPost(postId) {
				if(confirm("是否删除该岗位？")) {
					location.href = "<%=path%>/post/deletePostById.action?id=" + postId;
				}
			}

			function updatePost(postId) {
				if(confirm("是否修改该岗位？")) {
					location.href = "<%=path%>/post/selectPostById.action?id=" + postId;
				}
			}
		...
		<div class="am-cf admin-main">
			...
			<div class="admin-content">
				...
				<div class="am-cf am-padding">
					<div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">岗位基本信息</strong> <small></small></div>
				</div>
						...
						<div class="am-fl am-cf">
							<div class="am-btn-toolbar am-fl">
								<div class="am-btn-group am-btn-group-xs">
									<button type="button" class="am-btn am-btn-default" onclick="window.location.href='<%=path %>/jsp/post/addPost.jsp'"><span class="am-icon-plus"></span> 添加岗位</button>
					...
					<div class="am-u-md-3 am-cf">
						<div class="am-fr">
							...
							<div class="am-input-group am-input-group-sm">
								...
								<form action="<%=path %>/post/selectPostidById.action" method="post" onsubmit="return validate();">
									<span class="am-input-group-btn">
				  	<input type="text"  class="am-form-field" style="width: 150px"  placeholder="请输入岗位ID" name="postId" id="postId">&nbsp;&nbsp;&nbsp;
					...
					<div class="am-u-sm-12">
						<form class="am-form">
							<table class="am-table am-table-striped am-table-hover table-main">
								...
								<tr>
									<th class="table-id">岗位编号</th>
									<th class="table-title">岗位名称</th>
									<th class="table-type">类型</th>
									<th class="table-author">编制</th>
								...
								<c:forEach items="${allPost }" var="post">
									<tr>
										...
										<td>
											<c:if test="${post.post_type=='1' }">管理</c:if>
											<c:if test="${post.post_type=='2' }">技术</c:if>
											<c:if test="${post.post_type=='3' }">营销</c:if>
											<c:if test="${post.post_type=='4' }">市场</c:if>
										</td>
										<td>${post.strength }</td>
											...
											<td>
												<div class="am-btn-toolbar">
													<div class="am-btn-group am-btn-group-xs">
														<button type="button" class="am-btn am-btn-default am-btn-xs am-text-secondary" onclick="window.location.href='javascript:updatePost(${post.post_id})'"><span class="am-icon-pencil-square-o"></span>修改</button>
														<button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger" onclick="window.location.href='javascript:delPost(${post.post_id})'"><span class="am-icon-trash-o"></span>删除</button>
														<button type="button" class="am-btn am-btn-default am-btn-xs" onclick="window.location.href='<%=path%>/post/selectPostAllEmployeeById.action?postId=${post.post_id}'"><span class="am-icon-copy"></span>岗位下员工</button>
													</div>
												</div>
											</td>
									</tr>
