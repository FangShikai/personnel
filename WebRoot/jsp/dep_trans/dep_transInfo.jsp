...
<html>
	...
	-->
		...
		<script type="text/javascript">
			function deleteEmployeeDep_trans(dep_trans_id) {
				if(confirm("是否删除该员工的部门部门调转信息？")) {
					location.href = "<%=path%>/dep_trans/deleteEmployeeDep_trans.action?dep_trans_id=" + dep_trans_id;
				}
			}

			function showUpdateDep_trans(dep_trans_id) {
				if(confirm("是否修改该员工的部门部门调转信息？")) {
					location.href = "<%=path%>/dep_trans/showUpdateDep_trans.action?dep_trans_id=" + dep_trans_id;
				}
			}
			
			 function validate(){ 
          var obj = document.getElementById("emp_id");  
		...
		<div class="am-cf admin-main">
			...
			<div class="admin-content">
				...
				<div class="am-cf am-padding">
					<div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">部门调转信息</strong> <small></small></div>
				</div>
					...
					<div class="am-u-md-3 am-cf">
						<div class="am-fr">
							...
							<div class="am-input-group am-input-group-sm">
								...
								<form action="<%=path %>/dep_trans/selectEmployeeDep_trans.action" method="post" onsubmit="return validate();">
									<span class="am-input-group-btn">
				  	<input type="text"  class="am-form-field" style="width: 150px"  placeholder="请输入员工ID" name="emp_id" id="emp_id">&nbsp;&nbsp;&nbsp;
						...
						<form class="am-form">
							<table class="am-table am-table-striped am-table-hover table-main">
								...
								<tr>
									...
									<th class="table-author">调转后部门</th>
									<th class="table-date">调转类型</th>
									<th class="table-set">调转原因</th>
									<th class="table-author">备注</th>
									<th class="table-date">调动日期</th>
									<th class="table-set">操作</th>
								</tr>

								<c:forEach items="${Dep_transList }" var="dep_trans">
									<tr>

										<td>${dep_trans.dep_trans_id }</td>
										<td>${dep_trans.employee.emp_id }</td>
										<td>${dep_trans.employee.emp_name }</td>
										<td>${dep_trans.department.dep_name }</td>
										<td>
											<c:if test="${dep_trans.trans_type=='1' }">主动调动</c:if>
											<c:if test="${dep_trans.trans_type=='2' }">被动调动</c:if>
											<c:if test="${dep_trans.trans_type=='3' }">数据错误</c:if>
										</td>
										<td>${dep_trans.trans_reason }</td>
										<td>${dep_trans.note}</td>
										<td>
											<fmt:formatDate value="${dep_trans.trans_date }" pattern="yyyy-MM-dd" />
										</td>
										<td>
											<div class="am-btn-toolbar">
												<div class="am-btn-group am-btn-group-xs">
													<button type="button" class="am-btn am-btn-default am-btn-xs am-text-secondary" onclick="window.location.href='javascript:showUpdateDep_trans(${dep_trans.dep_trans_id})'"><span class="am-icon-pencil-square-o"></span>修改</button>
													<button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger" onclick="window.location.href='javascript:deleteEmployeeDep_trans(${dep_trans.dep_trans_id})'"><span class="am-icon-trash-o"></span>删除</button>
												</div>
											</div>
										</td>
									</tr>
