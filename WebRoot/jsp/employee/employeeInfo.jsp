...
<html class="no-js">
  ...
  <link rel="stylesheet" href="<%=path%>/jsp/assets/css/admin.css">
	...
	<script type="text/javascript">
		function delEmployee(emp_id){
			if(confirm("是否删除该员工？")){
				location.href="<%=path%>/employee/deleteEmployeeById.action?id="+emp_id;
			}
		}
		
		function updateEmployee(emp_id){
			if(confirm("是否修改该员工信息？")){
				location.href="<%=path%>/employee/showUpdateEmployee.action?id="+emp_id;
			}
		}
  ...
  <div class="admin-content">
    ...
    <div class="am-cf am-padding">
      <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">在职员工基本信息</strong> <small></small></div>
    </div>
        ...
        <div class="am-fl am-cf">
          <div class="am-btn-toolbar am-fl">
            <div class="am-btn-group am-btn-group-xs">
              <button type="button" class="am-btn am-btn-default" onclick="window.location.href='<%=path %>/employee/addEmployee_show.action'" ><span class="am-icon-plus"></span> 添加员工</button>
      ...
      <div class="am-u-md-3 am-cf">
        <div class="am-fr">
          ...
          <div class="am-input-group am-input-group-sm">
           <!--  <input type="text" class="am-form-field" placeholder="请输入员工ID">
                <span class="am-input-group-btn">
                  <button class="am-btn am-btn-default" type="button" >搜索</button>
                </span> -->
                
                
				<form action="<%=path %>/employee/selectEmployeeById.action" method="post" onsubmit="return validate();">
				  	 <span class="am-input-group-btn">
				  	<input type="text"  class="am-form-field" style="width: 150px"  placeholder="请输入员工ID" name="employeeId" id="employeeId">&nbsp;&nbsp;&nbsp;
      ...
      <div class="am-u-sm-12">
        <form class="am-form">
          <table class="am-table am-table-striped am-table-hover table-main">
              ...
              <tr>
               <th class="table-id">员工ID</th><th class="table-title">姓名</th><th class="table-type">性别</th>
                <th class="table-author">出生日期</th><th class="table-date">所在部门</th><th class="table-set">岗位</th>
              <th class="table-author">入职日期</th><th class="table-date">工作日期</th><th class="table-set">用工形式</th>
              <th class="table-set">民族</th><th class="table-set">电话</th><th class="table-set">邮箱</th><th class="table-set">操作</th>
              </tr>
           ...
           <c:forEach items="${allEmployee }" var="employee" >
            <tr>
              <td>${employee.emp_id }</td>
    			...
    			<td><c:if test="${employee.emp_sex=='1' }">男</c:if><c:if test="${employee.emp_sex=='2' }">女</c:if> </td>
    			...
    			<td><fmt:formatDate value="${employee.entry_date  }" pattern="yyyy-MM-dd"/></td>
    			<td><fmt:formatDate value="${employee.work_date }" pattern="yyyy-MM-dd"/></td>
    			<td>
    			<c:if test="${employee.work_type == '1' }">正式员工</c:if>
    			<c:if test="${employee.work_type == '2' }">临时员工</c:if>
    			</td>
              ...
              <td>
                <div class="am-btn-toolbar">
                  <div class="am-btn-group am-btn-group-xs">
                    <button type="button" class="am-btn am-btn-default am-btn-xs am-text-secondary" onclick="window.location.href='javascript:updateEmployee(${employee.emp_id })'"><span class="am-icon-pencil-square-o"></span>修改</button>
                    <button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger" onclick="window.location.href='javascript:delEmployee(${employee.emp_id })'"><span class="am-icon-trash-o"></span>删除</button> 
                  </div>
                </div>
              </td>
            </tr>
