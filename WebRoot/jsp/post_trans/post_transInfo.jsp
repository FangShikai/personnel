...
<html>
	...
	-->
		...
		<script type="text/javascript">
			function delPost_trans(post_transId) {
				if(confirm("是否删除该岗位调转信息？")) {
					location.href = "<%=path%>/post_trans/deletePost_transById.action?id=" + post_transId;
				}
			}

			function updatePost_trans(post_transId) {
				if(confirm("是否修改该岗位调转信息？")) {
					location.href = "<%=path%>/post_trans/selectPost_transById.action?id=" + post_transId;
				}
			}
		...
		<div class="am-cf admin-main">
			...
			<div class="admin-content">
				...
				<div class="am-cf am-padding">
					<div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">岗位调转信息</strong> <small></small></div>
				</div>
					...
					<div class="am-u-md-3 am-cf">
						<div class="am-fr">
							...
							<div class="am-input-group am-input-group-sm">
								...
								<form action="<%=path %>/post_trans/selectEmp_idById.action" method="post" onsubmit="return validate();">
						...
						<form class="am-form">
							<table class="am-table am-table-striped am-table-hover table-main">
								...
								<tr>
									...
									<th class="table-author">调转后岗位</th>
								...
								<c:forEach items="${allPost_trans }" var="post_trans">
									<tr>

										<td>${post_trans.post_trans_id }</td>
										<td>${post_trans.employee.emp_id}</td>
										<td>${post_trans.employee.emp_name}</td>
										<td>${post_trans.post.post_name}</td>
										<td>
											<c:if test="${post_trans.trans_type=='1' }">升职</c:if>
											<c:if test="${post_trans.trans_type=='2' }">降职</c:if>
											<c:if test="${post_trans.trans_type=='3' }">数据录入错误</c:if>
										</td>
										<td>${post_trans.trans_reason }</td>
										<td>${post_trans.note }</td>
										<td>
											<fmt:formatDate value="${post_trans.trans_date }" pattern="yyyy-MM-dd" /> </td>
										<td>
											<div class="am-btn-toolbar">
												<div class="am-btn-group am-btn-group-xs">
													<button type="button" class="am-btn am-btn-default am-btn-xs am-text-secondary" onclick="window.location.href='javascript:updatePost_trans(${post_trans.post_trans_id })'"><span class="am-icon-pencil-square-o"></span>修改</button>
													<button type="button" class="am-btn am-btn-default am-btn-xs am-text-danger" onclick="window.location.href='javascript:delPost_trans(${post_trans.post_trans_id })'"><span class="am-icon-trash-o"></span>删除</button>
												</div>
											</div>
										</td>
									</tr>
